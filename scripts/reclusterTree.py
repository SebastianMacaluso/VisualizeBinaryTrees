...
def dijMinPair(
    ...
    dij_list = [(np.sort((const_list_pt[pairs][k]) ** (2 * alpha))[0] * \
                 (np.arccos(np.dot(const_list[pairs][k][0],const_list[pairs][k][1])/
                            (epsilon + np.linalg.norm(const_list[pairs][k][0]) * np.linalg.norm(const_list[pairs][k][1])))) ** 2, k) for k in range(len(const_list[pairs]))]
    ...
    # print("Cos arg>1? =", np.sum([np.count_nonzero(np.absolute(np.sum(const_list[pairs][k][0] * const_list[pairs][k][1]) /
    #                         (np.sqrt(np.sum(const_list[pairs][k][0] ** 2)) * np.sqrt(
    #                           np.sum(const_list[pairs][k][1] ** 2))))> 1) for k in range(len(const_list[pairs]))]))
    #
    # cosines=[np.absolute(np.sum(const_list[pairs][k][0] * const_list[pairs][k][1]) /
    #                         (epsilon+np.sqrt(np.sum(const_list[pairs][k][0] ** 2)) * np.sqrt(
    #                           np.sum(const_list[pairs][k][1] ** 2)))) for k in range(len(const_list[pairs]))]
    # print("pos,value =", [(i,cosines[i]) for i in range(len(cosines)) if np.absolute(cosines[i])<0.99])



    # print("Cos arg>1? =", [np.absolute(np.sum(const_list[pairs][k][0] * const_list[pairs][k][1]) /
    #                         (np.sqrt(np.sum(const_list[pairs][k][0] ** 2)) * np.sqrt(
    #                           np.sum(const_list[pairs][k][1] ** 2)))) for k in range(len(const_list[pairs]))])
